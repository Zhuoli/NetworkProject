...
//***********************************
import java.util.Random; // Modification: Import random class
//***********************************
...
public class PeerCoordinator implements PeerListener
{
    ...
    //*****************************************************
    // Modification: Check period from 20 seconds to 10 seconds
    final static long CHECK_PERIOD = 10 * 1000; // 20 seconds
    //*****************************************************
	// set max_connections dynamic 
    static int MAX_CONNECTIONS = 60;

    //************************************************
    static int MAX_UPLOADERS = 8; // Modification: The max uploaders can be changed.
    //************************************************

    // Approximation of the number of current uploaders.
    // Resynced by PeerChecker once in a while.
    //************************************************
    ArrayList<Peer> uploaders = new ArrayList<Peer>();  // Modification: Not only memorize the number of uploaders
    //************************************************
        ...
        CoordinatorListener listener)
    {
        ...
        for (int i = 0; i < metainfo.getPieces(); i++) {
            if (!bitfield.get(i)) {
				// Here is not random, wanted pieces placed in order
                wantedPieces.add(i);
            }
        }
		// make wantedPieces random
    ...
    public void connected (Peer peer)
    {
        ...
        synchronized (peers) {
            if (peerIDInList(peer.getPeerID(), peers)) {
                ...
                {
                    unchokePeer();  //Modification: Synchronize the Uploaders.
                }
                //************************************************
    ...
    public void addPeer (final Peer peer)
    {
        ...
        if (need_more) {
			...
			// The fist piece each new peer gonna send
    ...
    void unchokePeer ()
    {
        ...
        while (it.hasNext()) {
            Peer peer = (Peer)it.next();
            //************************************************
            if (uploaders.size() < MAX_UPLOADERS && peer.isChoking()    //Modification: The uploaders are no longer a number any more.
                && peer.isInterested()) {
                if (!peer.isChoked()) {
        ...
        //************************************************
        if(interested.size() > 0)   // We gotta interested peers.
        {
            // Delete the least Up Rate peer. 
            if( MAX_UPLOADERS == uploaders.size() )
            {
                Peer tempPeer = uploaders.get(0);
                for(Peer uploadPeer : uploaders)
                {
                    long downloadSize = PeerCheckerTask.peerDownloadMap.get(uploadPeer);
                    long tempSize = PeerCheckerTask.peerDownloadMap.get(tempPeer);
                    if(downloadSize < tempSize)
                    {
                        tempPeer = uploadPeer;
                    }
                }
                uploaders.remove(tempPeer);
            }

            // Randomly uchoke one interested peer
            Random r = new Random();
            int unchokedNum = r.nextInt(interested.size());
            Peer unchokedPeer = interested.remove(unchokedNum);
            unchokedPeer.setChoking(false);
            uploaders.add(unchokedPeer);
            peers.remove(unchokedPeer);
            peers.add(unchokedPeer);
        }
        //************************************************
    }
        ...
        throws IOException
    {
        ...
        if (completed()) {
            client.interrupt();
            //**************************************************************
            System.exit(0);         //Modification: Exit as soon as the download is complete
            //**************************************************************
        }
        return true;
    }
    ...
    public void gotInterest (Peer peer, boolean interest)
    {
        if (interest) {
            synchronized (peers) {
                //***********************************
                if (uploaders.size() < MAX_UPLOADERS) {    //Modification: uploaders is no longer a number any more.
                    if (peer.isChoking()) {
                        uploaders.add(peer);   //Modification: add the peer.
                        peer.setChoking(false);
                        log.log(Level.FINER, "Unchoke: " + peer);
                    }
                }
    ...
    public void disconnected (Peer peer)
    {
        ...
        synchronized (peers) {
            ...
            if (peers.remove(peer)) {
                ...
                //***********************************
                synchronized(uploaders) // Synchronize the uploaders.
                {
                    unchokePeer();
                }
                //***********************************
