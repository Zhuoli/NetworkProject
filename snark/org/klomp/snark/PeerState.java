...
class PeerState
{
	...
	// currency initialized to 1, each time it send out one piece currency mins 1/
	// each time it get one piece from the same peer, currency pluss 1
	int currency;
	...
	// send out the piece the copeer requested
    void requestMessage (int piece, int begin, int length)
        throws IOException
    {
		// dont send out piece if currency is not enough
		if(this.currency <= 0)
			return;
